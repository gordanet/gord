...
func TestVirtualSelectedParentChain(t *testing.T) {
	...
	gord1, gord2, _, teardown := standardSetup(t)
	defer teardown()
	...
	chain1TipHash := consensushashing.BlockHash(gord1.config.NetParams().GenesisBlock)
	chain1TipHashString := chain1TipHash.String()
	const blockAmountToMine = 10
	for i := 0; i < blockAmountToMine; i++ {
		minedBlock := mineNextBlock(t, gord1)
	...
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, gord2)
	}
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := gord1.rpcClient.GetVirtualSelectedParentChainFromBlock(
