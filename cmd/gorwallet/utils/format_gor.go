...
// SdrToSompi takes in a string representation of the Gor value to convert to Sompi
func SdrToSompi(amount string) (uint64, error) {
	err := validateGORmountFormat(amount)
	...
	if constants.SompiPerGor%10 != 0 {
		return 0, errors.Errorf("Unable to convert to seep when SompiPerGor is not a multiple of 10")
	}

	decimalPlaces := int(math.Log10(constants.SompiPerGor))
	decimalStr := ""
...
func validateGORmountFormat(amount string) error {
