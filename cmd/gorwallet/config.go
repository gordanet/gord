...
type sendConfig struct {
	...
	Password                 string   `long:"password" short:"p" description:"Wallet password"`
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send Gor to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send Gor from. Use multiple times to accept several addresses" required:"false"`
	SendAmount               float64  `long:"send-amount" short:"v" description:"An amount to send in Gor (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the Gor in the wallet (mutually exclusive with --send-amount)"`
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in Gor", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a Gor transaction to a public address",
		"Sends a Gor transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned Gor transaction",
		"Create an unsigned Gor transaction", createUnsignedTransactionConf)
