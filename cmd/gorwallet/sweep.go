...
func sweep(conf *sweepConfig) error {
	...
	UTXOs, err := libgorwallet.GorwalletdUTXOsTolibgorwalletUTXOs(getExternalSpendableUTXOsResponse.Entries)
	if err != nil {
		return err
	}
	...
	for i, txID := range response.TxIDs {
		fmt.Printf("\t%s\n", txID)
		fmt.Println("\tSwept:\t", utils.FormatGor(splitTransactions[i].Outputs[0].Value), " GOR")
