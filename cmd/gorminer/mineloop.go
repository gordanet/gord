...
import (
	...
	"github.com/gordanet/gord/cmd/gorminer/templatemanager"
	...
	"github.com/gordanet/gord/domain/consensus/utils/pow"
...
func getBlockForMining(mineWhenNotSynced bool) (*externalapi.DomainBlock, *pow.State) {
	...
	for {
		...
		if !isSynced && !mineWhenNotSynced {
			if shouldLog {
				log.Warnf("Gord is not synced. Skipping current block template")
			}
...
func templatesLoop(client *minerClient, miningAddr util.Address, errChan chan error) {
	getBlockTemplate := func() {
		template, err := client.GetBlockTemplate(miningAddr.String(), "gorminer-"+version.Version())
		if nativeerrors.Is(err, router.ErrTimeout) {
