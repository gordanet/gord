...
supported gord messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Gord Message Overview

The gord protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which gord network it is a part of, its type, how big it is, and a checksum
...
of the currently supported gord messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using gord encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the gord messages are intended
...
and write gord messages. The following sections provide a quick overview of
...
The gord network is a magic number which is used to identify the start of a
message and which gord network the message applies to. This package provides
...
In order to unmarshall gord messages from the appmessage, use the ReadMessage
...
In order to marshall gord messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a gord peer. Example syntax to request addresses
