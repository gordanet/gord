/*
Package appmessage implements the gord appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported gor messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Gor Message Overview

The Gor protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which gor network it is a part of, its type, how big it is, and a checksum
...
of the currently supported gor messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using gor encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the gor messages are intended
...
and write gor messages. The following sections provide a quick overview of
...
The gor network is a magic number which is used to identify the start of a
message and which gor network the message applies to. This package provides
...
As discussed in the gor message overview section, this package reads
and writes gor messages using a generic interface named Message. In
...
In order to unmarshall gor messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a gor peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, gorNet)
	if err != nil {
...
In order to marshall gor messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a gor peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, gorNet)
	if err != nil {
		// Log and handle the error
	}
