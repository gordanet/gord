...
// Outpoint defines a gor data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new gor transaction outpoint point with the
...
// TxIn defines a gor transaction input.
type TxIn struct {
...
// NewTxIn returns a new gor transaction input with the provided
...
// TxOut defines a gor transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new gor transaction output with the provided
...
// MsgTx implements the Message interface and represents a gor tx message.
