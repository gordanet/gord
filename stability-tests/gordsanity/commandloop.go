...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("gordsanity-gord-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("gord", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "GORD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "GORD-STDERR")
