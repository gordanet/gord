...
import (
	...
	"github.com/gordanet/gord/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("gord-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("gord datadir: %s", dataDir)

	gordCmd, err := common.StartCmd("GORD",
		"gord",
	...
	spawn("startNode-gordCmd.Wait", func() {
		err := gordCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("gordCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes gord closes ungracefully
				log.Errorf("gordCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(gordCmd, "gordCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	gorMinerCmd, err := common.StartCmd("MINER",
		"gorminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(gorMinerCmd, "gorMinerCmd")
	return nil
}
